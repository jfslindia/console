...
class Console_Controller extends CI_Controller
{
    ...
    public function add_wash_image()
    {
        ...
        if($statecode == "all" || $statecode == ""){    
            $res = $this->Console_Model->add_wash_image($wash_image,$statecode,$cities,$brand_code,$site_url,$category);
        }else{
            $all_states = $this->Console_Model->get_states_sp();
            for($j=0;$j<sizeof($all_states);$j++){
                if($all_states[$j]['statecode'] == $statecode)
            ...
            for($i=0;$i<sizeof($cities);$i++){
                $res = $this->Console_Model->add_wash_image($wash_image,$state,$cities[$i],$brand_code,$site_url,$category);
            }
        }
        if ($res) {
            $data = array('status' => 'success', 'message' => 'successfully saved');
    ...
    public function check_tip(){
        ...
        $tip = $this->Console_Model->check_tip($device);
        if($tip == "")
            $data = 'success';
        else
            $data = 'failed';
   ...
   public function add_tip()
    {
        ...
        $category = $request['category'];
        $res = $this->Console_Model->add_tip($tip_image,$category);
        if ($res) {
            $data = array('status' => 'success', 'message' => 'successfully saved');
        } else {
            $data = array('status' => 'failed', 'message' => 'failed to save');
           ...
           $mob_no = (array_chunk($to_sent, 299));
            ...
            if($gcmids){
                ...
                for ($k = 0; $k < $users_no; $k++) {
                    if (!empty($gcmids[$k]))
                        array_push($regIds, $gcmids[$k]['gcmid']);
                }
                if ($device == 'android') {
                    if ($push_type == 'topic') {
                        $json = $push->getPush();
                    ...
                    if ($response) {
                        ...
                        for($s=0;$s < $users_no;$s++) {
                            if($regIds[$s] != "(null)"){
                                $mobile_number=$this->Console_Model->get_mobile_number($brand, $device,$regIds[$s]);
                                if($mobile_number != 0)
                                    $this->Console_Model->save_notification_details($id,$mobile_number,$status);
                            }
                        }
