...
class FS_API extends CI_Controller {
 ...
 public function get_storewise_prices()
 {
    ...
    $this->write_pg_response("CITY CODE : ".$city_code);
     ...
     if($prices){
        for($i=0;$i<sizeof($cities);$i++){
            if($cities[$i]['CITYCODE'] == $city_code){
                $locations = array("DELHI","GURGAON","GHAZIABAD","NOIDA","NEW DELHI"); // List of citiess with rates per Sq.ft
                $rate_message = "";
                for($k = 0; $k<sizeof($locations);$k++){
                    if(strtoupper($locations[$k]) == strtoupper($cities[$i]['CITYNAME'])){
                        $rate_message = "Carpet, Curtains blinds, Dhuree & Jamkhana are charged on sq.ft bases." ;
                        break;
                    }
                }
        ...
        if($rate_message == "")
            $rate_message = "Curtains, Carpet, Dhuree & Jamkhana are charged on sq.ft bases.";
        $data = array('prices' => $prices,'rate_message' => $rate_message);
      ...
      public function get_stores()
    {
    	...
    	for ($i = 0; $i < sizeof($stores); $i++) {
    		if (in_array($stores[$i]['BRANCHCODE'], $this->block_stores()) == FALSE) {
                ...
                $map_link = $this->FS_API_Model->get_location_link($stores[$i]['BRANCHCODE']);
