                ...
                <div id="offer_grid" class="uk-grid uk-grid-medium uk-child-width-1-3@m uk-grid-match">
                    <div class="uk-margin-bottom">
                        <div id="" class="uk-card uk-card-default">
                            ...
                            <div id="desktop_img_block" class="uk-card-media-top uk-padding uk-text-center">
                                <div class="js-upload" id="desktop_img_upload" uk-form-custom>
                            ...
                            <div id="mobile_img_block" class="uk-card-media-top uk-padding uk-text-center" style="display:none;">
                                <div class="js-upload" id="mobile_img_upload" uk-form-custom>
                                    <input type="file" multiple>
                                    <button uk-icon="icon: plus-circle; ratio: 3.5"
                                            class="uk-button uk-button-default uk-padding" type="button"
                                            tabindex="-1"></button>
                                </div>
                                <div class="hide">Height: 240px<br>Width: 672px<br></div>
                            </div>
                            <div class="uk-card-body">
                    ...
                    <?php if($wash_image != ""){ 
                          foreach($wash_image as $img) {
                            if(isset($img)){?>
                            <div class="uk-margin-bottom">
                                <?php if($img['category'] == "schedule-wash-desktop"){?>
                                    <h4>Desktop banner</h4>
                                <?php }else if($img['category'] == "schedule-wash-mobile"){?>
                                    <h4>Mobile banner</h4>
                                <?php } ?>
...
$img['state']; }?></h3>
                                    ...
                                    <div class="uk-card-footer">
                                        <p>
                                            <a onclick="delete_wash_image(<?php echo $img['id']; ?>,'<?php echo $img['category'];?>')"><span
...
<script>
    ...
    function delete_wash_image(id,category) {
    ...
    UIkit.upload('#desktop_img_upload', {
        ...
        completeAll: function (res) {
            ...
            if (response.moved_files.length > 0) {
                $('#desktop_img_block').html('');

                var img = $('<img />').attr({
                    'id': 'desktop_image',
                    'src': response.moved_files[0].link

                }).appendTo('#desktop_img_block');
    ...
    UIkit.upload('#mobile_img_upload', {
        url: base_url + 'index.php/console_controller/upload_image',
        multiple: false,
        concurrent: 1,
        beforeSend: function () {
        ...
        completeAll: function (res) {
            var response = JSON.parse(res.response);
            if (response.moved_files.length > 0) {
                $('#mobile_img_block').html('');
                var img = $('<img />').attr({
                    'id': 'mobile_image',
                    'src': response.moved_files[0].link

                }).appendTo('#mobile_img_block');
    ...
    function add_wash_image() {  
            if($('#banner_type').val() == "desktop")         
                var wash_image = $('#desktop_image').attr('src');
            else if($('#banner_type').val() == "mobile")
                var wash_image = $('#mobile_image').attr('src');
            ...
            UIkit.modal.confirm('Do you want to add this banner?').then(function () { 
                jQuery.ajax({
                    ...
                    data: {
                        ...
                        brand:brand,
                        site_url:site_url,
                        category: "schedule-wash-"+$('#banner_type').val()
                    },
                    xhr: function () {
                        var xhr = new window.XMLHttpRequest();
                        //Download progress
    ...
    $("#banner_type").change(function(){
        ...
        if(banner_type  == "desktop"){
            $('#desktop_img_block').removeAttr('style')
            $('#mobile_img_block').css({"display":"none"})
        }else {
            $('#mobile_img_block').removeAttr('style')
            $('#desktop_img_block').css({"display":"none"})
        }
    });
</script>
<style>
